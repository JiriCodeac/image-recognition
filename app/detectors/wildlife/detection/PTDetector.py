...
class PTDetector:
    ...
    DEFAULT_DETECTOR_LABEL_MAP = {
        LABEL_ANIMAL: 'Animal',
        LABEL_PERSON: 'Person',
        LABEL_VEHICLE: 'Vehicle'  # available in megadetector v4+
    }

    def __init__(self, model_path: str, force_cpu: bool = False, use_model_native_classes: bool = False):
        self.device = 'cpu'
        if not force_cpu:
    ...
    def generate_detections_one_image(self, img_original, image_id, detection_threshold, image_size=None):
            ...
            for det in pred:
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        api_box = self.convert_yolo_to_xywh(xywh)

                        conf = self.truncate_float(conf.tolist(), precision=self.CONF_DIGITS)
                        ...
                        detections.append({
                            'category': str(cls),
                            'confidence': conf,
                            'boundingBox': self.truncate_float_array(api_box, precision=self.COORD_DIGITS)
                        })
        ...
        # ...try
            ...
            result['failure'] = self.FAILURE_INFER
            print('PTDetector: image {} failed during inference: {}\n'.format(image_id, str(e)))
            traceback.print_exc(e)

        result['maxDetectionConfidence'] = max_conf
    ...
    def truncate_float_array(self, xs, precision=3):
        """
        ...
        return [self.truncate_float(x, precision=precision) for x in xs]
